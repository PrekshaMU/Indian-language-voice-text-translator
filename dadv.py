...
@st.cache_resource
def load_model():
    # Using a lightweight model to stay within Streamlit's memory limits
...
def show_visualization(fwd, back, semantic):
    ...
    df = pd.DataFrame({
        "Stage": ["Forward Translation", "Back Translation", "Semantic Similarity"],
...
# =====================================================
# MODE 2: VOICE TRANSLATOR (MODIFIED FOR WEB)
# =====================================================
    ...
    if audio_file:
        try:
            r = sr.Recognizer()
            # Processing the recorded file directly
            with sr.AudioFile(audio_file) as source:
                audio_data = r.record(source)
                text = r.recognize_google(audio_data, language=src_code)
...
# =====================================================
    ...
    if img_file:
        ...
        # GPU=False for Streamlit Cloud compatibility
